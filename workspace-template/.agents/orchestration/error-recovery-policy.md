...
Define how agents detect, diagnose, and recover from failures without entering infinite loops, silently swallowing errors, or escalating prematurely.

## Recovery cycle

```text
DETECT -> DIAGNOSE -> ISOLATE -> REPAIR -> LEARN
```
...
- build failures;
- verification evidence that contradicts acceptance criteria;
- repeated inability to achieve a sub-goal.
...
- record the failure pattern and solution in task evidence;
- determine whether a durable convention, rule, or ADR should prevent recurrence;
- update `.agents/state/retries.json` with the attempt record;
- log the event in `.agents/state/events.jsonl`.
